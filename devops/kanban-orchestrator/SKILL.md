...
### ⚠ Critical: workspace lifecycle — Dev → QA artifact passing

**The problem (confirmed 2026-06-06):** By default, Kanban uses `workspace_kind=scratch` — when a task is marked `done`, its workspace directory is **automatically cleaned up**. This breaks the multi-role handoff:

```
Dev (done) → workspace wiped → QA spawns → finds nothing to test → BLOCKED
```

QA (t_11106945) was blocked with: *"dev's source code and PRD no longer exist on disk (workspaces/t_907d0dbe and t_0fe221be have been cleaned up)"*

**Root cause:** The workspace cleanup daemon runs after task completion. Any dependent task that needs to read parent outputs will find empty directories.

**Three solutions — pick one per project:**

1. **Best — `workspace_kind=worktree`:** Add `workspace_kind=worktree` to each `kanban_create` call. Workspaces become git worktrees tied to the project's git repo, surviving across task completion. Dev's deliverables live in `/home/zcxx/.hermes/kanban/boards/<board>/worktrees/<task_id>/`.

2. **Acceptable — durable output path:** Instruct Dev in the task body to write deliverables to a **persistent location** (e.g. `/home/zcxx/hermes-projects/<project_name>/src/`) rather than the ephemeral workspace. QA reads from that path.

3. **Fallback — log recovery:** The only artifact that survives scratch cleanup is `logs/<task_id>.log`, which contains file diffs. If you must recover: `hermes kanban log <task_id>` shows the diff output (though it may be truncated — ~78 lines visible, 344+ lines omitted per file in large diffs).

**How to specify `workspace_kind` at task creation:**

```python
kanban_create(
    title="Dev: implement HTTP server",
    body="... write deliverables to /home/zcxx/hermes-projects/myproject/src/ ...",
    assignee="dev",
    workspace_kind="worktree",   # ← persists across task completion
)["task_id"]
```

**Verification after setup:** After Dev completes and QA starts, verify the artifact path exists:
```bash
ls /home/zcxx/.hermes/kanban/boards/<board>/worktrees/<dev_task_id>/src/
```
If the directory is empty or gone, the workspace_kind setting isn't working — QA will block.

---

**`--board` flag must be explicit in `-q` / subprocess mode.** When spawning the CEO orchestrator via `hermes chat -q` (non-interactive), the `boards switch` command does **not** persist across subprocess invocations. Every `kanban create` call must include `--board <board_name>` explicitly. Example full command:
```
hermes chat --profile platform-ceo --skills kanban-orchestrator -t hermes-cli -s <project_dir> -q \
  "项目 xxx 已创建。Kanban Board: 'myproject'. 请在 --board myproject 上创建 CEO→PM→Dev→QA 任务链。"
```
Without `--board`, tasks land in whatever board the gateway's current context points to (often `default`), making them invisible to the intended project board.
